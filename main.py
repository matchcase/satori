...
def process_ai(text):
    ...
    agent_executor.invoke({"input": cmd})
        
    # increase_match = re.search(r".*~increase font size(?: to (\d+))?", text, re.MULTILINE)
    # decrease_match = re.search(r".*~decrease font size(?: to (\d+))?", text, re.MULTILINE)
    # syntax_match = re.search(r".*~toggle syntax highlighting", text, re.MULTILINE)
